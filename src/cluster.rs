...
/// To query for clusters.surrealdb.aexol.com with kube, use Api<Document>.
...
#[kube(status = "ClusterStatus", shortname = "doc")]
...
async fn reconcile(doc: Arc<Cluster>, ctx: Arc<Context>) -> Result<Action> {
    ...
    let docs: Api<Cluster> = Api::namespaced(ctx.client.clone(), &ns);

    info!("Reconciling Cluster \"{}\" in {}", doc.name_any(), ns);
    finalizer(&docs, CLUSTER_FINALIZER, doc, |event| async {
        match event {
...
impl Cluster {
    ...
    async fn reconcile(&self, ctx: Arc<Context>) -> Result<Action> {
        ...
        let docs: Api<Cluster> = Api::namespaced(client, &ns);
        ...
        if name == "illegal" {
            return Err(Error::IllegalCluster); // error names show up in metrics
        }
        // always overwrite status object with what we saw
        let new_status = Patch::Apply(json!({
            "apiVersion": "surrealdb.aexol.com/v1",
            "kind": "Cluster",
...
pub async fn run(state: State) {
    ...
    let docs = Api::<Cluster>::all(client.clone());
...
#[cfg(test)]
mod test {
    use super::{Cluster, Context, error_policy, reconcile};
    use crate::{
    ...
    async fn finalized_doc_causes_status_patch() {
        let (testctx, fakeserver) = Context::test();
        let doc = Cluster::test().finalized();
    ...
    async fn finalized_doc_with_hide_causes_event_and_hide_patch() {
        let (testctx, fakeserver) = Context::test();
        let doc = Cluster::test().finalized().needs_hide();
    ...
    async fn finalized_doc_with_delete_timestamp_causes_delete() {
        let (testctx, fakeserver) = Context::test();
        let doc = Cluster::test().finalized().needs_delete();
    ...
    async fn illegal_doc_reconcile_errors_which_bumps_failure_metric() {
        let (testctx, fakeserver) = Context::test();
        let doc = Arc::new(Cluster::illegal().finalized());
        ...
        assert!(err.to_string().contains("IllegalCluster"));
    ...
    async fn integration_reconcile_should_set_status_and_send_event() {
        ...
        // create a test doc
        let doc = Cluster::test().finalized().needs_hide();
        let docs: Api<Cluster> = Api::namespaced(client.clone(), "default");
        ...
        let opts = ListParams::default().fields("involvedObject.kind=Cluster,involvedObject.name=test");
        let event = events
            .list(&opts)
            .await
            .unwrap()
